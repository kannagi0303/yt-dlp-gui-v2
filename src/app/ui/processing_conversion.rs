...
use super::common::form_row_label;
...
pub(super) fn conversion_nodes() -> [ConversionNode; 4] {
    [
        ConversionNode::Video,
        ConversionNode::Audio,
        ConversionNode::Container,
        ConversionNode::Subtitle,
    ]
}
...
pub(super) fn conversion_label(state: &AppState, node: ConversionNode) -> String {
    let key = match node {
        ConversionNode::Video => "processing.video",
        ConversionNode::Audio => "processing.audio",
        ConversionNode::Container => "processing.container",
        ConversionNode::Subtitle => "processing.subtitle",
    };
    state.ui_i18n_text_for_key(key).to_string()
}

pub(super) fn conversion_label_width(ui: &Ui, state: &AppState) -> f32 {
    let labels = [
        state.ui_i18n_text_for_key("processing.video"),
        state.ui_i18n_text_for_key("processing.audio"),
        state.ui_i18n_text_for_key("processing.container"),
        state.ui_i18n_text_for_key("processing.subtitle"),
    ];
    semantic_ui_metrics::settings_form_label_column_width_for_visible_texts(ui, &labels)
}
...
fn render_post_download_conversion(
    ...
    let label_width = conversion_label_width(ui, state);

    for node in conversion_nodes() {
        let label = conversion_label(state, node);
        form_row_label(ui, label_width, label.as_str(), |ui| {
            render_conversion_choices(ui, state, settings, node);
        });
    }
}

pub(super) fn render_conversion_choices(
    ui: &mut Ui,
    state: &AppState,
    settings: &mut TranscodeIntentSettings,
    node: ConversionNode,
    ...
    match node {
        ConversionNode::Video => render_video_codec_choices(ui, state, settings),
        ConversionNode::Audio => render_audio_choices(ui, state, settings),
        ConversionNode::Container => render_container_choices(ui, state, settings),
        ConversionNode::Subtitle => render_subtitle_choices(ui, state, settings),
    }
}
...
fn render_video_codec_choices(
    ...
    let options = [
        (
            ...
            state.ui_i18n_text_for_key("processing.choice.source"),
        ),
...
fn render_subtitle_choices(ui: &mut Ui, state: &AppState, settings: &mut TranscodeIntentSettings) {
    let preserve_text = state.ui_i18n_text_for_key("processing.subtitle.preserve");
    let embed_text = state.ui_i18n_text_for_key("processing.subtitle.embed");
    let burn_text = state.ui_i18n_text_for_key("processing.subtitle.burn");
    let options = [
...
fn choice_button(ui: &mut Ui, selected: bool, compatible: bool, label: &str) -> egui::Response {
    let mut button = egui::Button::new(label)
        .frame(true)
        .min_size(semantic_ui_metrics::processing_conversion_choice_button_minimum_size_from_current_control_metrics(ui));
