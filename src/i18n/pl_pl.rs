pub fn text<'a>(key: &'a str) -> &'a str {
    match key {
        ...
        "prepare.req.app_folder.description" => "Folder przenośny musi być zapisywalny, aby zapisywać ustawienia i dane pomocnicze.",
        "prepare.req.tools_folder.title" => "Folder narzędzi",
        "prepare.req.tools_folder.description" => "Wdrażanie zależności zapisuje tutaj yt-dlp, FFmpeg i Deno.",
        "prepare.req.deployment_temp.title" => "Tymczasowy folder wdrażania",
        "prepare.req.deployment_temp.description" => "Rozpakowywanie FFmpeg i Deno używa tego folderu tymczasowego.",
        "prepare.req.download_cache.title" => "Pamięć podręczna pobierania",
        "prepare.req.download_cache.description" => "Tryb pamięci podręcznej yt-dlp-gui zapisuje tutaj cache yt-dlp.",
        ...
        "prepare.req.output_folder.recommendation" => "Wybierz prawidłowy folder wyjściowy na ekranie głównym lub w Opcjach.",
        "prepare.req.config_file.title" => "Plik konfiguracji",
        "prepare.req.config_file.description" => "Aplikacja musi móc zapisać stan pominięcia Prepare oraz ścieżki narzędzi.",
        "prepare.req.generic_writable_recommendation" => "Wybierz folder z możliwością zapisu i sprawdź uprawnienia.",
        "prepare.req.config_not_folder" => "Ścieżka konfiguracji wskazuje folder. Wybierz ścieżkę do pliku.",
        "prepare.req.config_readonly" => "Plik konfiguracji jest tylko do odczytu.",
        "prepare.req.config_readonly_recommendation" => "Zezwól na zapis do pliku konfiguracji albo wybierz inny folder aplikacji.",
        "prepare.req.use_folder_path" => "Wybierz ścieżkę folderu zamiast ścieżki pliku.",
        "prepare.req.move_portable_folder" => "Przenieś aplikację do zapisywalnego folderu przenośnego.",
        "prepare.req.avoid_protected_folder" => "Nie umieszczaj aplikacji przenośnej w Program Files ani w folderze Windows. Przenieś ją do D:\\Portable lub folderu użytkownika.",
        "prepare.req.move_non_synced_folder" => "Przenieś ją do folderu niesynchronizowanego, np. D:\\Portable\\yt-dlp-gui-v2.",
        "prepare.req.drive_parent_exists" => "Upewnij się, że dysk i folder nadrzędny istnieją.",
        "prepare.req.permission_denied" => "Przenieś aplikację do zapisywalnego folderu przenośnego. Jeśli Pulpit/Dokumenty/Pobrane nadal zawodzą, może blokować ją Kontrolowany dostęp do folderów w Defenderze.",
        "prepare.req.file_in_use" => "Zamknij program, który może używać tego folderu, albo wybierz inny folder.",
        "prepare.req.free_disk_space" => "Zwolnij miejsce na dysku albo wybierz inny dysk.",
        "prepare.req.path_too_long" => "Przenieś aplikację do krótszej ścieżki, np. D:\\Portable\\yt-dlp-gui-v2.",
        "prepare.req.choose_writable_portable_folder" => "Wybierz wyraźnie zapisywalny folder przenośny i sprawdź ponownie.",
        ...
        "prepare.severity.short.required" => "Wymagane",
        "prepare.severity.short.recommended" => "Zalecane",
        "prepare.severity.short.optional" => "Opcjonalne",
